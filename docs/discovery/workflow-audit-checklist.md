...
- ~~Does eAdaptor Next inbound Basic Auth for interchange `HONEASHKG` read from the interchange config's own password field, or from a separately linked CW user/login record?~~ **Resolved 2026-08-29** — `HONEASHKG` belongs to the legacy `/eadaptor` endpoint, not eAdaptor Next at all; that's why it was never found in the `eAdaptorNext` config tables. See `docs/backlog/eadaptor-inbound-auth-401.md`.
