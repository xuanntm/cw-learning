...
|---|---|---|---|---|---|---|---|---|
| server1 (v1.4/1.5, wrong hostname) | 1.4/1.5 | `20.247.185.174` (matches `docs/config/uat-environment-overview.md`'s recorded "Virtual Environment IP") | `10.14.8.29` | OK (CNAME chain to `au2wpreadonly440l1.wisegrid.net` / `203.62.211.152`) | **OK** — `Microsoft SQL Server 2022 (RTM-CU25-GDR/KB5101347), Enterprise Edition, Windows Server 2019` | **FAIL** | **False** | SKIPPED |
| server2 (v1.5, wrong hostname) | 1.5 | `4.194.70.13` | `10.10.11.4` | OK | **OK** (same version as above) | **FAIL** | **False** | SKIPPED |
| server3 (v1.5, wrong hostname) | 1.5 | `52.230.84.213` | `10.10.11.7` | OK | **OK** (same version as above) | **FAIL** | **False** | SKIPPED |
| server1 (v1.6, corrected hostname) | 1.6 | `20.247.185.174` | `10.14.8.29` | OK | **OK** | **OK** (CNAME to `au2wtreadonly411l1.wisegrid.net` / `203.62.211.146`) | **True** | **OK** — same SQL Server build as PROD |
| server2 (v1.6, corrected hostname) | 1.6 | `4.194.70.13` | `10.10.11.4` | _(pending retest)_ | | | | |
| server3 (v1.6, corrected hostname) | 1.6 | `52.230.84.213` | `10.10.11.7` | _(pending retest)_ | | | | |
...
**✅ RESOLVED (2026-08-29): root cause was the wrong hostname, not a DNS/VNet gap.** CW's team confirmed the correct UAT hostname is `H56TRN.db.test.wisegrid.net` (missing `.test.` segment in what had been used until now). Retested from server1 with `db-connectivity-check.ps1` v1.6 — full success: DNS resolves via CNAME to `au2wtreadonly411l1.wisegrid.net` (`203.62.211.146`, mirroring PROD's `au2wpreadonly440l1.wisegrid.net` naming exactly), TCP connects, SQL login succeeds (`SQL Server 2022 RTM-CU25-GDR`, same build as PROD). The zone/peering-gap conclusion below was the best theory available from the evidence at the time but is now superseded — kept for reference. See `docs/backlog/uat-db-dns-resolution-failure.md` for full resolution details. **Track B is now unblocked** — proceed with the real traffic query using the corrected UAT connection details.

**CONCLUSION (2026-08-29, superseded above) — confirmed 3-for-3, UAT DNS failure is a zone/peering gap, not a whitelist or credential issue.** All 3 test servers (3 different public IPs, 3 different private subnets — ruling out any shared NAT-gateway confound) show the identical pattern: PROD (`H56PRD.db.wisegrid.net`) resolves via a CNAME to a differently-named zone (`au2wpreadonly440l1.wisegrid.net`) and logs in successfully; UAT (`H56TRN.db.wisegrid.net`) fails at the **DNS resolution step itself**, before any TCP/whitelist check would even apply. This rules out per-server IP whitelisting (which would block the TCP connect, not DNS resolution) and confirms the original VNet-wide DNS gap theory from 2026-08-23: UAT's DNS record likely lives in a private zone not linked/peered to any of these 3 servers' VNets.
